...
class Device():
    ...
    def _register(self, device):
        ...
        if self.get_device(address, 'temppv') == None:
            device_id = address + '_temppv'
            Domoticz.Debug('Creating domoticz device to handle temperature')
            Domoticz.Device(Unit=self.get_first_available_unit(), DeviceID=device_id, Name=name + ' - TemperaturePV', Type="Temp", Subtype="LaCrosse TX3").Create()
    ...
    def _update_state(self, state):
        ...
        if "TemSen" in state:
            self._update_device('temppv', int(state['TemSen']), str(state['TemSen']))
